...
uint32_t audio_buffer[AUDIO_BUFFER_SIZE] __attribute__((aligned(512)));
...
void setup_pwm_generator_dma_channel()
{
  ...
  dma_channel_configure(
      ...
      audio_buffer,                           // Read values from fade_a array
      512,                                    // nsamples words to transfer
      true                                    // Don't start yet.
  );
}

void send_audio(uint nsamples, bool irq0) {
  ...
  dma_channel_set_read_addr(pwm_dma_chan, audio_buffer, false);
